...
class Dataset_Train(Dataset):
    def __init__(self, cfg, update=None):
        self.cfg = cfg
        self.datalist_video = load_pickle(f'{self.cfg.dir["pre3_train"]}/datalist_{2}')
    ...
    def get_data_org(self, img_name):
        img = Image.open(f'{self.cfg.dir["dataset"]}/JPEGImages/{img_name}.jpg').convert('RGB')
        anno = load_pickle(f'{self.cfg.dir["pre0_train"]}/{img_name}')
        return img, anno
    ...
    def get_guidance_data(self, data):
        ...
        if len(data['approx_lanes']) > 1:
            approx_lanes = data['approx_lanes']
            sf = self.cfg.scale_factor['seg']
            sorted_idx = np.argsort(np.min(approx_lanes, axis=1))
            ref_idx = random.randint(0, len(sorted_idx) - 2)
            lane1 = approx_lanes[sorted_idx[ref_idx]]
            lane2 = approx_lanes[sorted_idx[ref_idx + 1]]
            case = random.randint(0, 1)
            if case == 0:
                w = random.uniform(0.1, 0.3)
            else:
                w = random.uniform(0.7, 0.9)
            guide_lane = np.zeros((len(lane1), 2), dtype=np.float32)
            guide_lane[:, 0] = lane1 * w + lane2 * (1 - w)
            guide_lane[:, 1] = self.cfg.py_coord
            for s in sf:
            ...
            idxlist = np.sum(data['seg_label'][s], axis=1).nonzero()[0]
            if len(idxlist) > 0:
                guide_mask_neg[:idxlist[0]] = 0
                guide_mask_neg[idxlist[-1]:] = 0
...
class Dataset_Test(Dataset):
    def __init__(self, cfg):
        ...
        self.datalist_video = load_pickle(f'{self.cfg.dir["pre3_test"]}/datalist_{2}')
    ...
    def get_data_org(self, idx):
        ...
        anno = load_pickle(f'{self.cfg.dir["pre0_test"]}/{self.datalist[idx]}')
        return img, anno
