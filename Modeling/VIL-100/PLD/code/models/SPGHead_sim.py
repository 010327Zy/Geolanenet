...
class SPGHead_sim(nn.Module):
    def __init__(self,
                 ...
                 cfg=None):
        super(SPGHead_sim, self).__init__()
        self.cfg = cfg
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        prior_xs = prior_xs.reshape(batch_size, num_priors, -1, 1)  # [4, 100, 72, 1]
        prior_ys = self.anchor_ys.repeat(batch_size * num_priors).view(
            batch_size, num_priors, -1, 1)
        # Change the value from [0, 1] to [-1, 1]
        prior_xs = prior_xs * 2. - 1.
        prior_ys = prior_ys * 2. - 1.
        grid = torch.cat((prior_xs, prior_ys), dim=-1).float()  # [4, 100, 72, 2]
        # Sample elements at coordinate positions in the grid from batch_features
        feature = F.grid_sample(batch_features, grid,   # [4, 100, 64, 72]
                                align_corners=True).permute(0, 2, 1, 3)

        feature = feature.reshape(batch_size * num_priors,  # [400, 64, 72, 1]
                                  self.anchor_feat_channels, self.n_offsets,
                                  1)
        roi = self.conv_downsample(feature)     # [400, 64, 72, 1]
        roi = roi.contiguous().view(batch_size*num_priors,-1)   # [400, 4608]
        roi = F.relu(self.fc_norm(self.downsample_layer(roi)))  # [400, 64]
        roi = roi.view(batch_size,num_priors,-1)    # [4, 100, 64]
        return roi
    ...
    def decode_theta_xy(self,heat_xy,heat_theta,topk):
        ...
        _,top_k_socre_idx = torch.topk(heat_nms_scores,k=topk,largest=True)     # 4000 -> 100
    ...
    def forward(self, x, **kwargs):
        ...
        device = x[-1].device
        # x16
        f_hm = x[self.hm_fmap_id]   # [4, 64, 40, 100]
        z = self.SPGU(f_hm)
        # hm (bs,1,hm_h,hm_w)
        hm = z['hm']    # [4, 1, 40, 100]
        shape_hm = z['shape']   # [4, 1, 40, 100]
        # param = z['parameter']
        theta_hm = shape_hm[:,0]    # [4, 40, 100]
        # x32
        # batch_features = x[-1]    # # [4, 64, 10, 25]
        batch_features,offset = self.ALAU(x[-1],torch.hstack([hm,shape_hm]))    # [4, 64, 10, 25]
        # batch_features,offset = self.ALAU(x[-1],shape_hm)

        self.anchors = torch.zeros((bs,self.num_anchor,2 + 2 + 2 + self.n_offsets),device = device) # [4, 100, 78]
        hms = torch.clamp(hm.sigmoid(), min=1e-4, max=1 - 1e-4)     # [4, 1, 40, 100]
        # theta_hms = torch.clamp(theta_hm.clone().sigmoid(), min=1e-4, max=1 - 1e-4)
        theta_hms = theta_hm    # [4, 40, 100]
        
        for bs_id,(hm_c,theta_hm_c) in enumerate(zip(hms,theta_hms)):
            # select top-100 pos and theta from the heatmap
            with torch.no_grad():
                ct_points,thetas = self.decode_theta_xy(hm_c,theta_hm_c,topk=self.num_anchor)   # [100, 2], [100]
                assert len(ct_points) == self.num_anchor

            # generate anchors according to the start points and angles
            anchors_per_batch = self.generate_anchors_in_one(starts=ct_points,angles=thetas)    # [100, 78]
        ...
        batch_anchor_features = self.pool_prior_features(batch_features,self.num_anchor,self.anchors[:,:,6:].clone())   # [4, 100, 64]
        batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels)    # [400, 64]
         # Predict
        cls_logits = self.cls_layer(batch_anchor_features)  # [400, 2]
        reg = self.reg_layer(batch_anchor_features)     # [400, 73]

        # Undo joining
        cls_logits = cls_logits.reshape(bs, -1, cls_logits.shape[1])    # [4, 100, 2]
        reg = reg.reshape(bs, -1, reg.shape[1])     # [4, 100, 73]
        ...
        reg_proposals[:, :, :2] = cls_logits    # [4, 100, 2]
        reg_proposals[:, :, 5:] += reg      # [4, 100, 73]
        proposals_list = self.nms(reg_proposals, nms_thres, nms_topk, conf_threshold)  # [4 * [100, 78]]
    ...
    def loss(self, output, batch, contr_ls):
        # import pdb;pdb.set_trace()
        hm_gt = batch['gt_hm']
        ...
        sim_loss = nn.CosineSimilarity(dim=1) ######
        cls_loss = 0
        reg_loss = 0
        length_loss = 0
        valid_imgs = len(targets)
        z1s,z2s,p1s,p2s = contr_ls ######
        # import pdb;pdb.set_trace()
        for proposals, target,hm_p,shape_hm_p,hm_g,shape_hm_g,shape_hm_m, in zip(proposals_list, targets,hm,shape_hm,hm_gt,shape_hm_gt,shape_hm_mask):
            ...
            with torch.no_grad():
                matched_row_inds, matched_col_inds = assign(    # pred, gt
        ...
        sim_loss = ((1-sim_loss(p1s,z2s)).mean()+(1-sim_loss(p2s,z1s)).mean())*0.5 ######
        # import pdb;pdb.set_trace()
        # cls reg l1 10:1
        # cls reg liou 1:6
        regw = 6
        ...
        if self.cfg.haskey('sim_loss_w'): ######
            sim_loss_w = self.cfg.sim_loss_w ######
        # print("--------------------------------------------------------------")
        # print("---------------------sim_loss_w:{}------------------------".format(sim_loss_w))
        # print("--------------------------------------------------------------")
        loss = cls_loss*cls_loss_w + reg_loss * regw + hm_loss * hmw + theta_loss*thetalossw + length_loss + sim_loss*sim_loss_w
        return {'loss': loss,
                'loss_stats':
                    {
                        'cls_loss': cls_loss, 
    ...
    def relocate_xy(self,proposals):
        ...
        with torch.no_grad():
            proposals_matched_positive_reg = proposals[:,6:].clone()    # [22, 72]
