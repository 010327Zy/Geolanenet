...
class Loss_Function(nn.Module):
    def __init__(self, cfg):
        ...
        self.coeff_weights = nn.Parameter(
            torch.tensor([1.3863, 0]),  # 初始logits对应softmax后的[0.8, 0.2]
            requires_grad=True
        )#####
    ...
    #     # coeff
    #     exclude_map = gt[t]['seg_label'][self.sf[0]].unsqueeze(3) * gt[t]['visit'][self.sf[0]].unsqueeze(3)
    #     mask = exclude_map[:, :, :, 0] == 1#####mask区域
    #     # 计算原始的IoU损失
    #     iou_loss = self.compute_IoU_loss(out[t]['coeff_map'].permute(0, 2, 3, 1), gt[t]['coeff_label'][self.sf[0]],
    #                                                    exclude_map=exclude_map)
        
        
    #     coeff_pred = out[t]['coeff_map'].permute(0, 2, 3, 1)  # [B,H,W,C]
    #     coeff_gt = gt[t]['coeff_label'][self.sf[0]]
    #     if torch.any(mask):
    #         coeff_mse = F.mse_loss(coeff_pred[mask], coeff_gt[mask])
    #     else:
    #         coeff_mse = torch.tensor(0.0, device=coeff_pred.device)

    #     # 动态权重混合
    #     weights = F.softmax(self.coeff_weights, dim=0)  # 确保权重和为1
    #     loss_dict['coeff'] = weights[0] * iou_loss + weights[1] * coeff_mse

    #     # 记录监控指标（不影响梯度）
    #     monitor_dict.update({
    #         'coeff/weight_iou': weights[0].detach(),
    #         'coeff/weight_mse': weights[1].detach(),
    #         'coeff/iou_raw': iou_loss.detach(),
    #         'coeff/mse_raw': coeff_mse.detach()
    #     })
    ...
    #         l_sum += loss_dict[key]
    #     loss_dict['sum'] = l_sum
    #     loss_dict.update({f'monitor/{k}': v for k,v in monitor_dict.items()})
    #     return loss_dict
    ...
    def compute_IoU_loss(self, out, gt, exclude_map):
        ...
        #out（32,96,160,6）  gt 像素系数图，目前是将系数图×U转化为车道线的iouloss    可以另外将系数图作一个对比损失加进去   iouloss也可以仿照adnet做Gloss #####
        ...
        d1 = torch.min(out_x_coords + e1, gt_x_coords + e1) - torch.max(out_x_coords - e1, gt_x_coords - e1)#对应位置的差 over(32,96,160,100)
        d2 = torch.max(out_x_coords + e1, gt_x_coords + e1) - torch.min(out_x_coords - e1, gt_x_coords - e1)#   union
        G = torch.clamp((d2 - 4*e1) / (d2 + 1e-9),min=0.,max=1.)#####

        d1 = torch.sum(d1, dim=3)#(32,96,160)
