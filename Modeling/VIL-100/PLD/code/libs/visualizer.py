...
class Visualize_cv(object):
    ...
    def display_single_results(self, path):
        # results = cv2.imread(path.replace(self.cfg.dir["out"], self.cfg.dir["model2"]).replace('val/display', 'test/display'))#测试时使用这一段
        results = cv2.imread(path.replace(self.cfg.dir["out"], self.cfg.dir["model2"]).replace('test/display', 'val/display'))##### 路径为'/train1/gzp/VIL-100/RVLD-main/Modeling/VIL-100/ILD_coeff/output/test/display/v239132710_1_0_1831/00000.jpg' 训练时使用这一段
    ...
    def display_for_test(self, batch, out, prev_frame_num, batch_idx, mode):
        # img
        self.update_image(batch['img'][batch_idx], name='img') #(32,3,384,640)
        self.update_image_name(batch['img_name'][batch_idx])#len = 32
        ...
        self.display_single_results(path=f'{self.cfg.dir["out"]}/{mode}/display/{dirname}/{filename}.jpg')#####这里其他代码都没有 注释掉的 有的话好像会报错
