...
from evaluation.evaluate_acc import LaneEval
from evaluation.evaluate_iou_official import LaneEval_CULane
...
def prepare_post_processing(cfg, dict_DB):
    ...
    dict_DB['save_pred_for_eval_acc'] = Save_Prediction_for_eval_acc(cfg=cfg)
...
def prepare_evaluation(cfg, dict_DB):
    ...
    dict_DB['eval_acc'] = LaneEval(cfg=cfg)
    dict_DB['eval_iou_official'] = LaneEval_CULane(cfg=cfg)
...
def prepare_training(cfg, dict_DB):
    ...
    if cfg.run_mode == 'train' and cfg.resume == False:
        rmfile(path=logfile)
        val_result = dict()
        val_result['acc'] = 0
        val_result['F1'] = 0
