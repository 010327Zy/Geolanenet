...
class Train_Process(object):
    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            out = dict()
            loss_per_clip = dict()
            for t in range(self.cfg.clip_length, -1, -1): #t= 2,1,0
                key_t = f't-{t}' # t-2 t-1 t-0
                out[key_t] = dict()
                if self.cfg.clip_length - t == 0:
                    self.vm.forward_for_dict_initialization()#初始化
                else:
                    self.vm.forward_for_dict_memorization()#转移信息至t-1帧，t-0帧初始化

                self.model.clip_idx = self.cfg.clip_length - t # self.model.clip_idx = 0,1,2
 
                self.vm.forward_for_dict_initialization_per_frame(t='t-0')#
                self.model_s.forward_for_encoding(batch[key_t]['img'])#传入t-2的数据
                ...
                self.vm.forward_for_dict_update_per_frame(self.model_s, mode='intra')#将t-2数据更新在memory=t-0的位置上，memory＋1
    ...
    def update_dataloader(self):
        if self.epoch == self.cfg.epoch_update2 and self.cfg.clip_length <= 1:
            ...
            dataset = Dataset_Train(cfg=self.cfg, update=True)
