...
class Dataset_Train(Dataset):
    ...
    def get_downsampled_label_seg(self, lanes, idx, sf):
        for s in sf:
            ...
            # 确保保存文件夹存在
            # output_folder = '/train1/gzp/VIL-100/RVLD-main/Modeling/VIL-100/ILD_coeff/code/datasets/output'  # 指定文件夹路径
            # if not os.path.exists(output_folder):
            #     os.makedirs(output_folder)
            
            # # 拼接完整路径
            # output_filename = f'seg_label_{s}_{idx + 1}.png'  # 保存的文件名，可以包含scale_factor和车道索引
            # output_path = os.path.join(output_folder, output_filename)

            # # 调试：打印保存路径
            # print(f"准备保存图像到：{output_path}")
            
            # # 保存当前的seg_label图像
            # success = cv2.imwrite(output_path, self.label['seg_label'][s])
    ...
    def get_downsampled_label_coeff(self, data, idx, sf):
        for s in sf:
            self.label['coeff_label'][s][self.label['seg_label'][s] == (idx + 1), :] = data#把 data 中的系数值（可能是车道线的多项式系数或其他表示车道特征的数据）赋给 coeff_label[s] 中对应的像素位置。


    def get_label(self, data):
        out = dict()
        self.label = dict()
        self.label['org_label'] = np.ascontiguousarray(np.zeros((self.cfg.height, self.cfg.width), dtype=np.uint8))#[384,640]
        self.label['seg_label'] = dict()
        self.label['coeff_label'] = dict()
        self.label['visit'] = dict()

        for s in self.cfg.scale_factor['seg']:#only[4]
            ...
            self.label['visit'][s] = np.zeros((self.cfg.height // s, self.cfg.width // s), dtype=np.float32)#初始化

        for i in range(len(data['lanes'])):
            lane_pts = data['lanes'][i]#点的数量
        ...
        for s in self.cfg.scale_factor['seg']:
            ...
            self.label['visit'][s] = np.float32(self.label['visit'][s] > 1)###### 1
        ...
        for s in self.cfg.scale_factor['seg']:
            ...
            self.label['visit'][s] = np.float32(self.label['visit'][s] == 0)######self.label['visit'][s] = np.float32(self.label['visit'][s] == 0)
