...
class Train_Process(object):
    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # model
            out = dict()
            self.model_s.forward_for_encoding(batch['img'])
            self.model_s.forward_for_squeeze()
            out.update(self.model_s.forward_for_classification())#out中更新model_s输出的概率图

            self.model.prob_map = self.model_s.prob_map[:, 1:] #只取前景 （b，1，h，w）概率图    但是size与model_s的输入（img.size）相同

            out.update(self.model.forward_for_regression())
            ...
            if self.epoch > 100 or (self.epoch + 1) % 10 == 0:
    ...
    def test(self):
        metric = self.test_process.run(self.model_s, self.model, mode='val')
